...
The implementation is fully self-contained here, done in python and Keras and actually matches pretty well with the pure Computer Vision steps expected in the below rubric.  
...
The implementation is fast: around 25 fps (on my PC: iCore7 + GPU GTX 980 TI). So much faster than a typical HOG+SVM CV approach while enabling to detect vehicles in both directions and is comparable to existing DL detection frameworks in terms of speed and ability to detect vehicles at different scales.  

In terms of write up, I am following the proposed rubric points template. Even if it was initially targetted for the HOG+SVM CV approach, apart for some specific details, eg the use of HOG features, in terms of steps and project breakdowns and implementation there is a 1-1 correspondence between the HOG+SVM CV approach and the Deep Learning CNN detection approach presented here.  
...
Raw RGB pixels are being used as input. Mean-var normalization is integrated into the fully convolutional classifier: so it will be handled on the GPU as well.  
...
The keyword here is **fully** convolutional Neural Network. Traditionally DL classifiers are made up of several convolutional layers followed by a few dense layers. Here the dense layers are replaced by convolutional layers as mentioned in the paper referred in (1) in the credits: Fully convolutional networks for Semantic Segmentation, https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_fcn.pdf. 

This approach is typically used as the basis of segmentation, to build heatmaps, and indeed this what we are going to do. But instead of performing segmentation we will use the heatmap built by this fully convolutional network, to detect objects, on the heat areas.  
...
The nice property of a convolutional filter or a fully convolutional neural network (built initially for classifying 64x64 images) is that it naturally slides over bigger images (720x1280 in our case) to create outputs that are no more 1 dimension (car not car for a 64x64 image) but a feature map or heatmap that predicts, for every 64x64 possible positions within the 720x1280 image, the car probability.  
...
The amount of parameters here is relatively low for a Neural Network classifier. The training is fast, and the validation set accuracy is 99.6%.  
90% of the provided data is used for the training set, and 10% is used for the validation set.  
...
-  The fully convolutional neural network 64x64 classifier naturally slides over an image that is much bigger than 64x64 to produce a feature map (or heatmap) output. As represented below.


By sliding with our 64x64 classifier over (720, 1280, 3) images, we get an output that is (83, 153, 1); mainly dividing size by 8 as we are using an 8x8 pooling and the filters have been defined so the borders remain the same (before pooling). So we get a 2D output where the (x,y) pixel value corresponds to a number between -1 and 1. The closer the pixel value to 1, the highest the probability that there is a car located within the m-rectangle (mx=8*x, my=8*y, mw=64, mh=64) in the original input image.  
...
- the area from 400:550 y-coordinates: is searched at a scale of 0.5.   
...
It also helps the detection to run faster by not sliding over the full image for every possible scales.  

Note that to search at a scale of, e.g., 2, we resize the original image by making it 2 times smaller and then run the 64x64 classifier over it.  
Similarly to search at a scale of, e.g., 0.5, we resize the original image by making it 2 times bigger and then run the 64x64 classifier over it.  
...
Then as explained in the lectures, overlapping boxes are grouped and overlayed over the detected object.  
...
To optimize the accuracy of the classifier I used:
- DL CNN classifier  
...
- Cropping image for different limited searches. Upper parts of the image look for smaller cars and bottom parts of the image look for bigger cars.    
...
- for cars driving in the same direction than our car: 3 consecutive frames are being used and their heatmaps summed. Then a threshold of 3*3 overlaps is used to identify areas where we will drow a bounding box.
...
The key step here, using fully convolutional networks to build heatmaps, is usually applied in a context of Images segmentation. Here it has been used to derive object detections. I am pretty happy with the result both in terms of accuracy (for vehicles driving in both directions, even if the training data was provided for cars seen only from behind !!!) and in terms of speed: around 25 fps.   
...
Concerning of next steps and improvements I really would like to study in details the SquezzeDet framework and architecture: 
